/**
 * Imports wave one of the 4 Sep 2026 10.04 PM batch: the five clubs that had no
 * listing on the store at all — Atletico Madrid, Sporting CP, Al-Nassr,
 * Borussia Dortmund and Celtic.
 *
 * The other five teams in that export (Portugal, Argentina, England, Italy and
 * the Netherlands) are NOT in this wave. Italy already carries 20 listings and
 * the Netherlands 17, so those need a dedupe pass against the live storefront
 * first, and the England and Argentina blocks contain photo albums that still
 * have to be grouped by eye.
 *
 * Identification, grouping, the two flagged assumptions and pricing all live in
 * `scripts/batch-0904c-manifest.json`. Photographs are staged into
 * `import/batch-0904c` by `scripts/stage-0904c.py`, which must run first —
 ...
 *
 * Usage: node scripts/import-batch-0904c.mjs [--apply] [--limit N] [--team X]
 */
...
const BATCH_TAG = "supplied-batch-2026-09-04c";
const manifest = JSON.parse(readFileSync(new URL("./batch-0904c-manifest.json", import.meta.url), "utf8"));
