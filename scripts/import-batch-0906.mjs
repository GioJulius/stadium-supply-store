 ...
 * `scripts/batch-0906/teams/<NN>-<slug>.json`, each written once by eye and then
 * enriched by `dedupe-0906.py` and `apply-0906-names.mjs`. Those team files ARE
 * the manifest; the shared tiers, sizes and batch tag live in
 * `scripts/batch-0906/batch.json`.
 *
 * Photographs are resolved BY FILENAME out of `import/batch-0906/<team-slug>/`,
 * which `apply-0906-names.mjs` populates. There is no positional `_index.json`
 ...
 *
 * Usage: node scripts/import-batch-0906.mjs [--apply] [--limit N] --team "Real Madrid"
 *        node scripts/import-batch-0906.mjs [--apply] --all
 */
...
const DIR = fileURLToPath(new URL("./batch-0906/", import.meta.url));
const TEAMS_DIR = join(DIR, "teams");
const MIRROR = fileURLToPath(new URL("../import/batch-0906/", import.meta.url));
...
const photoPath = (data, file) => {
  ...
  if (!existsSync(p)) throw new Error(`${data.team}: staged file missing, run apply-0906-names.mjs --apply first: ${p}`);
  return p;
};

// Prove every photograph resolves before touching Shopify.
for (const { data } of targets) {
  if (data.status === "named") throw new Error(`${data.team} has not been renamed yet — run apply-0906-names.mjs --apply`);
