/**
 * Imports the six kits the client posted to Instagram on 30 Aug 2026.
 *
 * The client sent 30 photographs over WhatsApp with a price list rather than a
 * supplier feed, so there is no scrape to run: `scripts/new-kits-manifest.json`
...
const manifest = JSON.parse(readFileSync(new URL("./new-kits-manifest.json", import.meta.url), "utf8"));
...
const files = (await readdir(MEDIA_DIR)).filter(f => f.startsWith("stadium.supply_") && f.endsWith(".jpg")).sort();
if (files.length !== 14) throw new Error(`expected 14 Instagram photos, found ${files.length}`);
...
const DESCRIPTION = title =>
  `<p>${title}, imported to order from our supplier. Sizes run small — see the size guide before choosing.</p>`;
...
for (const item of manifest.products) {
  ...
  const sizes = manifest.sizes[item.tier];
  ...
  const result = await gql(
    `mutation($input:ProductSetInput!){ productSet(input:$input, synchronous:true){ product{ id handle } userErrors{field message} } }`,
    {
      input: {
        ...
        descriptionHtml: DESCRIPTION(item.title),
        tags: [...item.tags, "Football", "Mapped Media", "ig-drop-2026-08-30"],
